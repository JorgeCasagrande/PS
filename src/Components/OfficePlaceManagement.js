...
import { officePlaces, users } from './data'; // Usa el archivo de datos actualizado
...
const OfficePlaceManagement = () => {
  ...
  const [places, setPlaces] = useState(officePlaces);
  ...
  const data = {
    labels: ['Lugares Libres', 'Lugares Ocupados'],
    datasets: [
      {
        data: [
          placesFilter.filter((place) => place.assignment.length === 0).length,
          placesFilter.filter((place) => place.assignment.length != 0).length,
        ],
  ...
  return (
    <Container maxWidth={false} style={{ marginTop: '2rem' }}>
      <Typography variant="h4" gutterBottom>
        Gestión de Lugares en la Oficina
      </Typography>
